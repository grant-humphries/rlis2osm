...
insert into osm_sts_staging (geom, st_prefix, st_name, st_type, st_direction, 
	...
	select rs.geom, 
		...
		case when prefix ~* 'N' then 'North'
			when prefix ~* 'NE' then 'Northweast'
			when prefix ~* 'E' then 'East'
			when prefix ~* 'SE' then 'Southeast'
			when prefix ~* 'S' then 'South'
			when prefix ~* 'SW' then 'Southwest'
			when prefix ~* 'W' then 'West'
			when prefix ~* 'NW' then 'Northwest'
			else null end,
		...
		case when ftype ~* 'ALY' then 'Alley'
			when ftype ~* 'AVE' then 'Avenue'
			when ftype ~* 'BLVD' then 'Boulevard'
			when ftype ~* 'BRG' then 'Bridge'
			when ftype ~* 'CIR' then 'Circle'
			when ftype ~* 'CORR' then 'Corridor'
			when ftype ~* 'CRST' then 'Crest'
			when ftype ~* 'CT' then 'Court'
			when ftype ~* 'DR' then 'Drive'
			when ftype ~* 'EXPY' then 'Expressway'
			when ftype ~* 'FRTG' then 'Frontage Road'
			when ftype ~* 'FWY' then 'Freeway'
			when ftype ~* 'HTS' then 'Heights'
			when ftype ~* 'HWY' then 'Highway'
			when ftype ~* 'LN' then 'Lane'
			when ftype ~* 'LNDG' then 'Landing'
			when ftype ~* 'PKWY' then 'Parkway'
			when ftype ~* 'PL' then 'Place'
			when ftype ~* 'PT' then 'Point'
			when ftype ~* 'RD' then 'Road'
			when ftype ~* 'RDG' then 'Ridge'
			when ftype ~* 'RR' then 'Railroad'
			when ftype ~* 'SQ' then 'Square'
			when ftype ~* 'ST' then 'Street'
			when ftype ~* 'TER' then 'Terrace'
			when ftype ~* 'TRL' then 'Trail'
			when ftype ~* 'VIA' then 'Viaduct'			
			when ftype ~* 'VW' then 'View'
		...
		case when direction ~* 'N' then 'North'
			...
			when direction ~* 'EB' then 'Eastbound'
			when direction ~* 'S' then 'South'
			when direction ~* 'SB' then 'Southbound'
			when direction ~* 'W' then 'West'
